...
/**
 * Vista amigable de categorías que reemplaza /question/bank/managecategories/category.php
 * Muestra el nombre del curso y sus categorías con el contenido estándar (editar, eliminar, mover).
 *
...
use local_dragndrop\question_category_object;

require_login();
core_question\local\bank\helper::require_plugin_enabled(helper::PLUGINNAME);

list($thispageurl, $contexts, $cmid, $cm, $module, $pagevars) =
    question_edit_setup('categories', '/local/dragndrop/category.php');
...
if ($param->moveupcontext || $param->movedowncontext) {
    require_sesskey();
    $catid = $param->moveupcontext ?: $param->movedowncontext;
...
if ($param->delete) {
    ...
    if ($questionstomove) {
        ...
        if ($moveform->is_cancelled()) {
            ...
            list($tocategoryid, ) = explode(',', $formdata->category);
...
if ($qcobject->catform->is_cancelled()) {
    ...
    redirect($thispageurl);
} else if (!empty($param->delete) && !$questionstomove && confirm_sesskey()) {
...
// Nombre del curso destacado arriba
echo html_writer::tag('h2', format_string($COURSE->fullname), ['class' => 'local-dragndrop-course-name']);

// Contenido estándar: editar categoría, mover preguntas, o lista de categorías
if ($param->edit !== null || $qcobject->catform->is_submitted()) {
    ...
    // Lista de categorías con edit, delete, move up/down (contenido por defecto)
